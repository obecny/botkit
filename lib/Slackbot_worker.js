...
module.exports = function(botkit, config) {
    ...
    var bot = {
        ...
        api: slackWebApi(botkit, _.clone(config)),
    ...
    var botApi = slackWebApi(botkit, bot.config);
    ...
    bot._startDM = function(task, user_id, cb) {
        botApi.im.open({ user: user_id }, function(err, channel) {
    ...
    bot.send = function(message, cb) {
        ...
        if (message.attachments || message.icon_emoji ||
            ...
            botApi.chat.postMessage(slack_message, function(err, res) {
                if (err) {
                    cb && cb(err);
                } else {
                    cb && cb(null, res);
